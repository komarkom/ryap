...
type calculable = Num of int
	| Bool of bool

let sprint_calculable c = 
	let rec sprint = function 
		| Num n -> sprintf "%d" n
		| Bool b -> sprintf "%b" b
	in sprintf "%s" (sprint c)

type expr = Val of calculable
	| Var of string
...
let sprint_expr e = 
	let rec sprint = function
		| Val c -> sprintf "%s" (sprint_calculable c)
		| Var c -> sprintf "%s" c
...
let eval_operator e = 
	...
	| Write s -> printf "%s\n" (sprint_calculable (eval_expr env s)); env
...
let _ =
	...
	let pp = BinOp (Var "y", Val (Num 1), pls) in
	...
	let iInc = BinOp(Var "i", Val (Num 1), pls) in
	let printIAndInc = Seq (Write (Var "i"), Assign("i", iInc)) in
	let whileCycle = Mywhile (BoolIntBinOp(Var "i", Val (Num 10), lesser), printIAndInc)  in
	let prog = Seq (Assign("i", Val (Num 0)), whileCycle) in
	let pr = sprint_prog prog in
	printf "program:\n\n\n%s\n\n\n" pr;
